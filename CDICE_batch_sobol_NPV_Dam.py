...
S1 = np.zeros((26,stop-start))
S1_conf = np.zeros((26,stop-start))
S2 = np.zeros((26,26,stop-start))
S2_conf = np.zeros((26,26,stop-start))
ST = np.zeros((26,stop-start))
ST_conf = np.zeros((26,stop-start))

#Load in data
problem = read_param_file('SALib-master/SALib_Input.txt',' ')
Y_matrix = np.loadtxt('Dam.txt')
...
#Load in data
problem = read_param_file('SALib-master/SALib_Input.txt',' ')
Y_matrix = np.loadtxt('NPV_Dam.txt')
N = np.size(Y_matrix,0)
cnt = 0
for i in range(start,stop):
    #Y = np.zeros((N))
