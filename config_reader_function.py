...
def config_reader(filename_in,filename_out):
    import csv
    import numpy as np
    ...
    with open(filename_in) as csvfile:
        ...
        for row in reader:
            ...
            cnt = cnt+1
    np.savez(filename_out,fix_index=fix_index,fix_names=fix_names,fix_nominal=fix_nominal,num_fix=num_fix,var_index=var_index,var_names=var_names,var_lo_bounds=var_lo_bounds,var_hi_bounds=var_hi_bounds,num_vars=num_vars,nominal=nominal)